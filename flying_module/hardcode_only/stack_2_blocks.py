...
'''
def takeoff_go_to(commander, TAKEOFF_HEIGHT=0.75, TAKEOFF_VELOCITY=0.2):
    commander.take_off(TAKEOFF_HEIGHT, TAKEOFF_VELOCITY)
    time.sleep(2)
    commander.go_to(0,0,0.75)
    time.sleep(2)
...
def land_ground(commander, LAND_VELOCITY):
    commander.go_to(0.5,0.5,0.75,0.3)
...
def pickup_block(commander, block_location_set, block_location, block_set_velocity):
    commander.go_to(block_location_set[0],block_location_set[1],0.2,0.1)
    time.sleep(0.25)
...
def stack_blocks(commander, block_location_set, block_location, dropoff_location_set, dropoff_location, flight_velocities):
    ...
    dropoff_location_block2 = [dropoff_location[0], dropoff_location[1], dropoff_location[2]+0.01]
    dropoff_location_set_b1ock2 = [dropoff_location_set[0], dropoff_location_set[1], dropoff_location_set[2]+0.01]
    move_block(commander, block_location_set, block_location, dropoff_location_set_b1ock2, dropoff_location_block2, flight_velocities)
...
block_location_set = [0,0.15,0.14] 
block_location = [0,0.15,0.04]
dropoff_location_set = [0.25,0.25,0.15]
dropoff_location = [0.25,0.25,0.03]
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        time.sleep(2)
        stack_blocks(commander, block_location_set, block_location, dropoff_location_set, dropoff_location, flight_velocities)